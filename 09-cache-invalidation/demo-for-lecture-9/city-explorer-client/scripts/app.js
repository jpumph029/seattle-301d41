...
function getResource(resource, location) {
  $.get(`${__API_URL__}/${resource }`, {data: location})
    .then((data, textStatus, jqXHR) => {
      if (jqXHR.status === 201) {
        console.log(`New location resource created in database for ${location.search_query}`);
      } else if (jqXHR.status === 200) {
        console.log(`Location data retrieved from database for ${location.search_query}`);
      } else {
